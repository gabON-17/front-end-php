...
function mascara(input) {
  ...
  if (isNaN(v[v.length - 1])) {
    // Acessa o último caractere da string e verifica se é número.
...
form.addEventListener("submit", async (e) => {
  e.preventDefault();
  const url = "http://localhost/api/server.php/students";

  const name = document.getElementById("aluno").value;
  const birth = document.getElementById("data").value;
  const sex = verifySex(document.getElementsByName("sex"));
  ...
  const nearby = document.getElementById("bairro").value;
  const city = document.getElementById("estado").value;
